...
export const PURITY_FINENESS: Record<Purity, number> = {
  // カラット定義上の24/24。実際の純金地金は99.99%等だが、刻印K24に対する
  // 名目値として1.0を使う(シルバーだけは価格基準がSv1000のため0.999)。
  k24: 1,
  k22: 0.916,
  k21_6: 0.9,
  // K20は基準が2つある。カラット定義なら20/24=83.3%、日本で使われていた
  // ホールマーク(日章旗と菱形の刻印)の表示は835=83.5%。国内の品物は後者の
  // 刻印を持つことがあるため835を採るが、K20としか刻印が無い品では83.3%が近い。
  // 差は0.2ポイントで、画面には採用した品位を表示している。
  k20: 0.835,
  k18: 0.75,
  k14: 0.585,
  k10: 0.416,
